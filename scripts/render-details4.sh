...
render_report_header() {
    ...
    if [ ! -f ${OVERVIEW} ] ; then
       ...
       echo "| Specification | auto | ctx | rdf | html | rspc | shcl | web | uml | mer | trans | meta | stake |" > ${OVERVIEW}
       echo "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |" >> ${OVERVIEW}
...
autotranslatefiles() {
    ...
    if [ -f "${INPUTTRANSLATIONFILE}" ]; then
        ...
        if ! node /app/autotranslate.js -i ${INPUTTRANSLATIONFILE} -s ${AZURETRANLATIONKEY} -m ${PRIMELANGUAGE} -g ${GOALLANGUAGE} -o ${OUTPUTTRANSLATIONFILE} &>> ${REPORTFILE} ; then
            echo "RENDER-DETAILS: failed"
            execution_strickness
        else
            echo "RENDER-DETAILS: translation file succesfully updated"
